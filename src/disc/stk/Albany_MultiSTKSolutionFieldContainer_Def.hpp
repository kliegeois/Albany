...
MultiSTKSolutionFieldContainer<Interleaved>::MultiSTKSolutionFieldContainer(
          ...
          fieldContainer_)
{
  ...
  for (int vec_num = 0; vec_num < solution_vector.size(); vec_num++) {
    if (solution_vector[vec_num].size() ==
      ...
      VFT* solution =
          &metaData->declare_field<VFT>(stk::topology::NODE_RANK, name);
      stk::mesh::put_field_on_mesh(
          *solution, metaData->universal_part(), neq_, nullptr);
#ifdef ALBANY_SEACAS
      ...
      VFT* solution = &metaData->declare_field<VFT>(
      ...
      for (int i = 0; i < solution_vector[vec_num].size(); i += 2) {
        if (solution_vector[vec_num][i + 1] == "V") {
          ...
          stk::mesh::put_field_on_mesh(
              *solution, metaData->universal_part(), len, nullptr);
#ifdef ALBANY_SEACAS
          stk::io::set_field_role(*solution, Ioss::Field::TRANSIENT);
#endif
          ...
          len = 1;  // scalar
          accum += len;
          SFT* solution = &metaData->declare_field<SFT>(
              stk::topology::NODE_RANK, solution_vector[vec_num][i]);
          stk::mesh::put_field_on_mesh(
              *solution, metaData->universal_part(), nullptr);
#ifdef ALBANY_SEACAS
          stk::io::set_field_role(*solution, Ioss::Field::TRANSIENT);
#endif
          sol_vector_name[vec_num].push_back(solution_vector[vec_num][i]);
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::fillVector(
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::fillSolnVector(
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::fillSolnMultiVector(
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::saveVector(
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::saveSolnVector(
    ...
    const Teuchos::RCP<const Thyra_VectorSpace>& node_vs)
{
  ...
  *out << "IKT WARNING: calling MultiSTKSolutionFieldContainer::saveSolnVectorT with "
          "soln_dotT, but "
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::saveSolnMultiVector(
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::saveResVector(
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::transferSolutionToCoords()
{
  const bool MultiSTKSolutionFieldContainer_transferSolutionToCoords_not_implemented =
      true;
  TEUCHOS_TEST_FOR_EXCEPT(
      MultiSTKSolutionFieldContainer_transferSolutionToCoords_not_implemented);
}

template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::fillVectorImpl(
    Thyra_Vector&                                field_vector,
    const std::string&                           field_name,
    stk::mesh::Selector&                         field_selection,
    const Teuchos::RCP<const Thyra_VectorSpace>& field_node_vs,
...
template <DiscType Interleaved>
void
MultiSTKSolutionFieldContainer<Interleaved>::saveVectorImpl(
    const Thyra_Vector&                          field_vector,
    const std::string&                           field_name,
    stk::mesh::Selector&                         field_selection,
    const Teuchos::RCP<const Thyra_VectorSpace>& field_node_vs,
